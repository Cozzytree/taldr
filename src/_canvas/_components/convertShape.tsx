import {BombIcon, Circle, LucideIcon, Octagon, Square, Star, Triangle} from "lucide-react";
...
const ConvertShape = ({canvas} : {canvas :  React.MutableRefObject<CanvasClass | null>}) => {

	const handleConvertShape = (type : shapeType, iteration? : number, inset? :number) => {
		...
		if (iteration) {
			...
			canvas.current.canvasShapes[shape].props.xRadius = canvas.current.canvasShapes[shape].props.w * 0.5;
			canvas.current.canvasShapes[shape].props.iteration = iteration;
			canvas.current.canvasShapes[shape].props.inset = inset;
			canvas.current.canvasShapes[shape].props.h = canvas.current.canvasShapes[shape].props.w
		} else {
			canvas.current.canvasShapes[shape].type = type;
		}
		canvas.current.draw();
	...
	return (
		<Menubar className={"bg-foreground/5"}>
			<MenubarMenu>
			<MenubarTrigger className={"w-full text-xs flex justify-between"}>Shapes <Square/></MenubarTrigger>
			<MenubarContent side={"left"} className={"z-[100] grid grid-cols-3 gap-1 bg-foreground/5"}>
				{shapes.map((s) =>
					<MenubarItem onClick={() => handleConvertShape(s.label, s?.iteration, s?.inset)} className={"w-8 h-8"}>
						<s.Icon key={s.label}/>
					</MenubarItem>
				)}
			</MenubarContent>
