  ...
  switch (shape.type) {
    ...
    case "text":
      if (direction) {
        ...
        findConnectionAndMove({ allShapes: shapes, shape });
        const xSide =
          mouseX > resizeShape.props.x
            ? mouseX - resizeShape.props.x
            : resizeShape.props.x - mouseX;
        const ySide =
          mouseY > resizeShape.props.y
            ? mouseY - resizeShape.props.y
            : resizeShape.props.y - mouseY;
        shape.props.fontSize = Math.max(5, xSide * 0.1 + ySide * 0.1);
      }
      break;
    case "pencil":
      if (direction)
    ...
    case "others":
      shape.props.xRadius =
        mouseX > resizeShape.props.x + (resizeShape.props.xRadius ?? 0)
          ? mouseX - (resizeShape.props.x + (resizeShape.props.xRadius ?? 0))
          : resizeShape.props.x + (resizeShape.props.xRadius ?? 0) - mouseX;
      findConnectionAndMove({ allShapes: shapes, shape });
