...
const lineConnection = ({
  ...
  allShapes.forEach((p, i) => {
    ...
    if (
      ...
      if (smallesShape == null || allShapes[smallesShape].props.w > p.props.w) {
        smallesShape = i;
      }
    }
  });
    ...
    if (s.props.startShape) {
      if (s.props.startShape.shapeId === shape.id) {
        s.props.startShape.followPoint = {
          x: shape.props.x + (s.props.startShape.xPer / 100) * shape.props.w,
          y: shape.props.y + (s.props.startShape.yPer / 100) * shape.props.h,
        };
